...
export default function NewTypePaiementModal(props) {
  const { setToastAction } = useApp();
  const [isSubmitting, setIsSubmitting] = useState(false);

  const initialValues = {
    ID_TYPE_PAIEMENT: props.data?.ID_TYPE_PAIEMENT,
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const url = `/type_paiements${data.ID_TYPE_PAIEMENT ? `/${data.ID_TYPE_PAIEMENT}` : ''}`

      const response = await fetchApi(url, {
        method: data.ID_TYPE_PAIEMENT ? 'PUT' : 'POST',
  ...
  return (
    <Dialog
      header={props.data ? `Modifier le type paiement` : `Nouveau type paiement`}
      ...
      <form onSubmit={handleSubmit} className="flex flex-column gap-2">
        ...
        <div className="row">
            <div className="col-md-12">
                ...
                value={data.MONTANT}
                placeholder=""
                className={`w-100 is-invalid ${errors?.MONTANT
                    ? "p-invalid"
                    : ""
                    }`}
