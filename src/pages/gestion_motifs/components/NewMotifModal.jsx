...
export default function NewMotifModal(props) {
  const { setToastAction } = useApp();
  const [isSubmitting, setIsSubmitting] = useState(false);

  const initialValues = {
    ID_MOTIF: props.data?.ID_MOTIF,
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const url = `/motifs${data.ID_MOTIF ? `/${data.ID_MOTIF}` : ''}`

      const response = await fetchApi(url, {
        method: data.ID_MOTIF ? 'PUT' : 'POST',
  ...
  return (
    <Dialog
      header={props.data ? `Modifier le motif` : `Nouveau motif`}
