...
import NewTypeDocumentModal from "./components/NewTypeDocumentModal";
import { type_documents_routes_items } from "@/routes/gestion_type_documents/type_documents_routes";

export default function TypeDocumentList() {
  const [selectAll, setSelectAll] = useState(false);
  const [loading, setLoading] = useState(true);
  const [totalRecords, setTotalRecords] = useState(1);
  const [type_documents, setTypeDocuments] = useState([]);
  ...
  const deleteItems = async (itemsIds) => {
    try {
      ...
      form.append("IDS_TYPE_DOCUMENT", JSON.stringify(Array.isArray(itemsIds) ? itemsIds : [itemsIds.ID_TYPE_DOCUMENT]));
      ...
      setToastAction({
        severity: "success",
        summary: "Type document supprimé",
        detail: res.message,
        life: 3000,
      })
  ...
  useEffect(() => {
    ...
    setBreadCrumbAction([type_documents_routes_items.type_documents])
    document.title = type_documents_routes_items.type_documents.name;
  ...
  return (
    <>
      ...
      {addVisible && <NewTypeDocumentModal visible={addVisible} setVisible={setAddVisible} data={inViewMenuItem} fetchTypeDocuments={fetchTypeDocuments} />}

      <div className="px-4 py-3 main_content">
        <div className="d-flex align-items-center justify-content-between">
          <h1 className="mb-3">Type documents</h1>
          <Button
            label="Nouveau"
