...
export default function ProfilListPage() {
  ...
  const deleteItems = async (itemsIds) => {
    try {
      ...
      form.append("ID_PROFILS", JSON.stringify(Array.isArray(itemsIds) ? itemsIds : [itemsIds.ID_PROFIL]));
                  ...
                  let items = [
                    {
                      ...
                      },
                      {
                        ...
                        template: item => (
                          <div className='p-menuitem-content px-3'>
                            <Link onClick={e => handleDeletePress(e, inViewMenuItem || selectedItems.map(e => e.ID_PROFIL))} className="flex align-items-center p-2" style={{ textDecoration: "none", color: '#3d3d3d', cursor: 'pointer' }}>
